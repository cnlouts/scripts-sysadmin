...
def do_create_user(first,last,pw,email)
  ...
  result = system("/usr/sbin/useradd -G managers,developers,www-data -s /bin/bash -b /home/managers -m -p '#{ pwd }' #{ uname }")
    if result
      puts "#{ uname } created!"
      system("/bin/chmod 0750 /home/managers/#{ uname }")
      FileUtils.cp_r "/root/.ssh", "/home/managers/#{ uname }/"
      FileUtils.rm "/home/managers/#{ uname }/.ssh/authorized_keys"
      FileUtils.chown_R "#{ uname }", "#{ uname }", "/home/managers/#{ uname }/.ssh" 
